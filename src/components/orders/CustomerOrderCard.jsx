...
const CustomerOrderCard = ({ order, isArabic, currencies, onSelect }) => {
  ...
  return (
    <Card
      variant="default"
      className="group relative overflow-hidden p-2.5 text-start transition-all duration-200 hover:-translate-y-0.5 hover:border-[color:rgb(var(--color-primary-rgb)/0.36)] hover:shadow-[var(--shadow-medium)] sm:p-3"
    >
      ...
      <div className="flex items-start gap-3">
        <div className="h-12 w-12 shrink-0 overflow-hidden rounded-[0.95rem] border border-[color:rgb(var(--color-border-rgb)/0.9)] bg-[color:rgb(var(--color-card-rgb)/0.82)] sm:h-14 sm:w-14">
          {order.productImage ? (
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2.5">
            <div className="min-w-0">
              <p className="line-clamp-2 text-[13px] font-semibold text-[var(--color-text)] sm:text-sm">{order.productName}</p>
          ...
          <div className="mt-2.5 flex flex-wrap items-center gap-1.5 text-[10px] sm:text-[11px]">
          ...
          <div className="mt-2.5">
            <Button
              ...
              className="h-9 w-full rounded-[0.95rem] text-xs sm:h-10"
